...
# Native LD_PRELOAD shim, preloaded into the wineserver boot by run.sh (issue #24,
# Radmin 2.1 port-reuse NAT traversal). Same no-strip rule as rvpn_dnsfix.so.
$(BUILD)/rvpn_reuseport.so: src/rvpn_reuseport.c | $(BUILD)
	$(CC) $(CFLAGS) -shared -fPIC -o $@ $< -ldl
...
check-build-artifacts:
	@for f in tap_bridge rvpnnetmp.sys adapter_hook.dll rvpn_launcher.exe netsh.exe netsh64.exe drvinst.exe rvpn_dnsfix.so rvpn_reuseport.so; do \
